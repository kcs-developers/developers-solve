pipeline {
  ...
  environment {
    ...
    dockerHubRegistry = 'startdreamteam/developers-solve-test'
    dockerHubRegistryCredential = 'docker_cre'
    applicationGitAddress = 'https://github.com/kcs-developers/developers-solve.git'
  ...
  stages {
    ...
    stage('k8s manifest file update') {
      steps {
        ...
        sh "sed -i 's@${dockerHubRegistry}:.*@${dockerHubRegistry}:${currentBuild.number}@g' deploy/solve/deploy-solve.yml"
        sh "chmod +x deploy/solve/deploy-solve.yml"
        sh "git add deploy/solve/deploy-solve.yml"
        sh "git status"
